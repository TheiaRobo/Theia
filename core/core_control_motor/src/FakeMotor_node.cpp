...
{
	static float right_speed_old;
	static float left_speed_old;
	...
	//Filtering
	//time_init = time;
//	for (int i = time_init, i < time_init+5; i+=1) {
//	}

	right_speed = (right_speed + right_speed_old)/2;
	...
	left_speed = (left_speed + left_speed_old)/2;
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "FakeMotor_node");
	ros::NodeHandle n;
	ros::Subscriber sub2 	= n.subscribe("/motion/PWM", 100, change_motor_speed);
