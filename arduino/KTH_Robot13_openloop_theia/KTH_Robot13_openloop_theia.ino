 ...
 * 	| "/motion/Abs_Encoders"  send Absolute Encoder value
...
#include <differential_drive/Abs_Encoders.h>
...
differential_drive::Encoders delta_encoder ;
differential_drive::Abs_Encoders absolute_encoder;
differential_drive::AnalogC Amsg ;

ros::Publisher delta("/motion/Encoders", &delta_encoder);  // Create a publisher to "/motion/Encoders" topic
ros::Publisher absolute("/motion/Abs_Encoders", &absolute_encoder);
...
void messagePWM( const differential_drive::PWM &cmd_msg){
  ...
  delta_encoder.delta_encoder1 = encoder1_loc-encoder1_old ;
  delta_encoder.delta_encoder2 = encoder2_loc-encoder2_old ;
  
  absolute_encoder.encoder1 = -encoder1_loc; // Wheels are mounted in reverse.
  absolute_encoder.encoder2 = -encoder2_loc;
  
  delta_encoder.timestamp = time - time_old ;
  absolute_encoder.timestamp = time - time_old;
  
  delta.publish(&delta_encoder);
  absolute.publish(&absolute_encoder);
...
void setup()  {  
         ...
         nh.advertise(delta);  // advertise on delta
         nh.advertise(absolute);
